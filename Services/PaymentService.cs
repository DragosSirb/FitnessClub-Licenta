...
namespace FitnessClub.Services
{
    public class PaymentService
    {
        ...
        private async Task ProcessShopOrder(IDictionary<string, string> metadata)
        {
            ...
            await _db.SaveChangesAsync();
            await AddNotification(memberId, "Comandă plasată",
                $"Comanda ta #{order.Id} a fost plasată cu succes. Total: {total} RON.",
        ...
        private async Task ProcessSubscription(IDictionary<string, string> metadata)
        {
            ...
            await _db.SaveChangesAsync();
            await AddNotification(memberId, "Abonament activat",
                $"Abonamentul tău {plan.Name} a fost activat cu succes.",
        ...
        private async Task ProcessTrainerSession(IDictionary<string, string> metadata)
        {
            ...
            await _db.SaveChangesAsync();
            await AddNotification(memberId, "Ședință rezervată",
                $"Ședința ta de antrenament a fost rezervată pentru {date:dd MMM yyyy} la ora {startTime}.",
        ...
        private async Task ProcessGroupClassEnrollment(IDictionary<string, string> metadata)
        {
            ...
            await _db.SaveChangesAsync();
            await AddNotification(memberId, "Înscriere la clasă confirmată",
                "Ai fost înscris cu succes la clasă.",
        ...
        private async Task ProcessEventEnrollment(IDictionary<string, string> metadata)
        {
            ...
            await _db.SaveChangesAsync();
            await AddNotification(memberId, "Înscriere la eveniment confirmată",
                "Ai fost înscris cu succes la eveniment.",
