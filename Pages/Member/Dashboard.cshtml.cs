...
namespace FitnessClub.Pages.Members
{
    ...
    public class DashboardModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            var expiredSubs = await _context.MemberSubscriptions
                .Where(s => s.MemberId == CurrentMember.Id && s.Status == SubscriptionStatus.Active && s.EndDate < DateTime.Now)
                .ToListAsync();
            foreach (var s in expiredSubs) s.Status = SubscriptionStatus.Expired;
            if (expiredSubs.Any()) await _context.SaveChangesAsync();
        ...
        private async Task<MemberSubscription?> GetActiveSubscription(int memberId)
        {
            return await _context.MemberSubscriptions
                .Include(s => s.Plan)
                .Where(s => s.MemberId == memberId && s.Status == SubscriptionStatus.Active && s.EndDate >= DateTime.Now)
