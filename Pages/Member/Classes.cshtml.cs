...
namespace FitnessClub.Pages.Member
{
    ...
    public class ClassesModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            var query = _context.GroupClassSchedules
            ...
            if (!string.IsNullOrEmpty(ClassFilter))
                query = query.Where(s => s.GroupClass.Name == ClassFilter);

            if (PriceFilter == "free")
                query = query.Where(s => s.GroupClass.IncludedInSubscription);
            else if (PriceFilter == "paid")
                query = query.Where(s => !s.GroupClass.IncludedInSubscription);

            UpcomingSchedules = await query
                .OrderBy(s => s.Date)
