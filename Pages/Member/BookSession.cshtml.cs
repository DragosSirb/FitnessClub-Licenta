  ...
  public class BookSessionModel : PageModel
  {
    ...
    public async Task<IActionResult> OnPostCancelAsync(int sessionId)
    {
      ...
      if (session != null && session.Date >= DateTime.Today)
      {
        ...
        TempData["Success"] = "Ședința a fost anulată cu succes.";
      }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
      ...
      if (Input.PaymentMethod == PaymentMethod.Online && price <= 0)
      {
        TempData["Error"] = "Acest antrenor nu are un preț configurat. Te rugăm să alegi plata la sală sau contactează recepția.";
        return RedirectToPage(new { trainerId = Input.TrainerId });
      }
      ...
      if (Input.PaymentMethod == PaymentMethod.Online)
      {
        ...
        {
          TempData["Error"] = $"Eroare plată online: {ex.StripeError?.Message ?? ex.Message}. Încearcă plata la sală.";
          return RedirectToPage(new { trainerId = Input.TrainerId });
        }
      ...
      else
      {
        ...
        {
          UserId = user.Id,
          Title = "Cerere ședință trimisă",
          Message =
            $"Ședința ta cu {trainer.FirstName} {trainer.LastName} pe {Input.Date:dd MMM yyyy} la ora {Input.StartTime}" +
            $" a fost trimisă și așteaptă confirmare.",
        ...
        TempData["Success"] = "Ședință rezervată! Se așteaptă confirmarea antrenorului.";
